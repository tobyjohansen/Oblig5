...
if __name__ == '__main__':
    ...
    while True:
        ...
        client.send_data(p1.rect)
        if playerNumber == 0:
            p2_position_data = client.client.recv(1024)
            p2_pd = p2_position_data.decode("utf_8")
            p2.rect = eval(p2_pd)
            print(eval(p2_pd))
            print("Test")
        if playerNumber == 1:
            p1_position_data = client.client.recv(1024)
            p1_pd = p1_position_data.decode("utf_8")
            p1.rect = eval(p1_pd)
